...
Singleton {
    ...
    Process {
        id: fetchKeyboardLayoutsProcess
        command: ["niri", "msg", "-j", "keyboard-layouts"]

        stdout: StdioCollector {
            id: fetchKeyboardLayoutsCollector
            onStreamFinished: {
                try {
                    root.applyKeyboardLayouts(JSON.parse(fetchKeyboardLayoutsCollector.text))
                } catch (e) {
                    console.warn("NiriService: Failed to parse keyboard layouts:", e)
                }
            }
        }
    }
